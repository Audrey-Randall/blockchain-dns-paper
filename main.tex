...
To 
analyze the two different types of naming systems, we select several of each 
type and perform two analyses. The first is a summary of how the system works, 
including how the smart contracts interact with each other, etc. The second is 
an estimate of the types of names and their uses that users are actually 
resolving. Discovering which names users are resolving in blockchain-based 
...
Unstoppable?}, which may be the only use some customers require. Second, while 
...
from the ``Old'' resolvers, usually to the current default resolvers.} To fully 
resolve a 
...
\section{Further B-Root Measurements}
